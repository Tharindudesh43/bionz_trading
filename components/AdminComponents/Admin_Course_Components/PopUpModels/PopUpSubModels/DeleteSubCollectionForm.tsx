...
interface DeleteSubCollectionConfirmationProps {
  SubCollectionID: String;
  onDeleteSuccess?: () => void;
  onClose?: () => void;
}

const DeleteSubCollectionConfirmation = ({
  SubCollectionID,
  onDeleteSuccess,
  onClose,
}: DeleteSubCollectionConfirmationProps) => {
  const collectionId = SubCollectionID;
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!res.data.success) {
        throw new Error(res.data.message || "Failed to delete sub collection.");
      }
      if (onDeleteSuccess) {
        onDeleteSuccess();
      }
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-2xl border border-red-300 dark:border-red-700 relative">
      ...
      <h2 className="text-2xl font-bold mb-6 text-red-600 dark:text-red-400 flex items-center">
        ...
        Confirm Sub Collection Deletion
      </h2>
...
export default DeleteSubCollectionConfirmation;
