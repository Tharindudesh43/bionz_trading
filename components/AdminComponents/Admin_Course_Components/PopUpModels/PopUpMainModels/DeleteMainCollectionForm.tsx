...
interface DeleteMainCollectionConfirmationProps {
  MainCollectionID: String;
  onDeleteSuccess?: () => void;
  onClose?: () => void;
}

const DeleteMainCollectionConfirmation = ({
  MainCollectionID,
  onDeleteSuccess,
  onClose,
}: DeleteMainCollectionConfirmationProps) => {
  const collectionId = MainCollectionID;
  ...
  const handleDelete = async () => {
    ...
    if (!collectionId) {
      setError("Error: Collection ID is missing.");
      return;
    }
    setIsLoading(true);
    setError(null);
    try {
      ...
      if (!res.data.success) {
        throw new Error(res.data.message || "Failed to delete collection.");
      }
    ...
    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-2xl border border-red-300 dark:border-red-700 relative">
      ...
      <div className="space-y-3">
        ...
        >
          ...
          {isLoading ? "Deleting Collection..." : "Yes, Delete Permanently"}
        </button>
        ...
        <button
          ...
          type="button"
          onClick={onClose}
          className="w-full cursor-pointer flex items-center justify-center px-4 py-3 bg-gray-300 text-gray-800 font-semibold rounded-lg hover:bg-gray-400 transition duration-200 shadow-lg"
        >
          Cancel
        </button>
      </div>
...
export default DeleteMainCollectionConfirmation;
